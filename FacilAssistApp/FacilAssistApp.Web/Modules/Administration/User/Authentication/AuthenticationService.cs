...
namespace FacilAssistApp.Modules.Administration.User.Authentication
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        private bool ValidateExistingUser(ref string username, string password, UserDefinition user)
        {
            ...
            if (user.IsActive != 1)
            {
                if (Log.IsInfoEnabled)
                    Log.Error(String.Format("Inactive user login attempt: {0}", username), GetType());
            ...
            {
                Log.Error("Error on directory access", ex, GetType());
            ...
            {
                Log.Error("Error while updating directory user", ex, GetType());
                return true;
            }
        }
        ...
        private bool ValidateFirstTimeUser(ref string username, string password)
        {
            ...
            {
                Log.Error("Error on directory first time authentication", ex, GetType());
                return false;
            }
            ...
            {
                Log.Error("Error while importing directory user", ex, GetType());
                return false;
            }
        }
    }
